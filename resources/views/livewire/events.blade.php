                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($payment->or_file_path)
                                            @php
                                                /** @var \Illuminate\Filesystem\FilesystemAdapter $evS3 */
                                                $evS3    = Storage::disk('s3');
                                                $evOrUrl = $evS3->url($payment->or_file_path);
                                                $evOrExt = strtolower(pathinfo($payment->or_file_path, PATHINFO_EXTENSION));
                                            @endphp
                                            <button type="button" x-data
                                                    @click="$dispatch('open-or-modal',{url:'{{ $evOrUrl }}',ext:'{{ $evOrExt }}'})"
