<div>
<section class="overflow-hidden border border-white/10 bg-zinc-900/75 shadow-[0_18px_40px_rgba(0,0,0,0.28)] backdrop-blur">
            ...
            <table class="w-full text-left text-sm">
                <thead class="border-b border-white/10 bg-zinc-950/80 text-zinc-400">
                    <tr>
                        ...
                        <th class="px-6 py-4 font-medium">Event Fee</th>
                        <th class="px-6 py-4 font-medium">Payment Status</th>
                    ...
                    @else
                        @forelse ($participants as $participant)
                            ...
                            <tr class="transition hover:bg-emerald-500/[0.04]">
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-[0.12em] {{ $rsvpBadgeClasses }}">
                                        {{ $rsvpLabel }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-[0.12em] {{ $paymentBadgeClasses }}">
                                ...
                                <td class="px-6 py-4">
                                    <select
                                        ...
                                        class="rounded-xl border border-white/10 bg-zinc-950 px-3 py-2 text-xs text-zinc-200 outline-none transition focus:border-emerald-400 focus:ring-2 focus:ring-emerald-500/20"
                                    >
                        ...
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                ...
                                <div class="mt-2 flex flex-wrap gap-2">
                                    <span class="inline-flex rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide {{ $rsvpBadgeClasses }}">
                                        {{ $rsvpLabel }}
                                    </span>

                                    <span class="inline-flex rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide {{ $paymentBadgeClasses }}">
                                        {{ str($paymentStatus)->headline() }}
                                    </span>
                                </div>
                                ...
                                <div class="mt-2 text-xs text-zinc-400">
                                    ...
                                    <span>
                                        {{ $participant->payment_updated_at ? \Carbon\Carbon::parse($participant->payment_updated_at)->format('M d, Y h:i A') : 'No payment update yet' }}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="shrink-0">
                                <select
                                    wire:change="updatePaymentStatus({{ $participant->id }}, $event.target.value)"
                                    class="rounded-lg border border-white/10 bg-zinc-950 px-2.5 py-2 text-[11px] text-zinc-200 outline-none transition focus:border-emerald-400 focus:ring-2 focus:ring-emerald-500/20"
                                >
                                    <option value="unpaid" @selected($paymentStatus === 'unpaid')>Unpaid</option>
                                    <option value="paid" @selected($paymentStatus === 'paid')>Paid</option>
                                    <option value="waived" @selected($paymentStatus === 'waived')>Waived</option>
